					...
					if(Words.length != 0){
						while(i < Words.length){
							if(Words[i].length()>=1){
								...
								if(i==0){
									Text	OutKey = new Text ("_START_"+" "+Words[i]+"\t"+AuthorLastName);
									context.write(OutKey, OutValue);
								}
								else if(i== Words.length-1){
									Text OKey = new Text (Words[i-1]+" "+Words[i]+"\t"+AuthorLastName);
									context.write(OKey, OutValue);
									Text	OutKey = new Text (Words[i-1]+" "+"_END_"+"\t"+AuthorLastName);
									context.write(OutKey, OutValue);
								}
								else{
									Text OutKey = new Text (Words[i-1]+" "+Words[i]+"\t"+AuthorLastName);
									context.write(OutKey, OutValue);
								}
								}
								i++;
